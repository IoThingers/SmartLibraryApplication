...
public class SignInActivity extends AppCompatActivity implements
    ...
    public void onConnected(Bundle bundle) {
        Log.i(TAG, "sharique onConnected");
    ...
    public void onResult(People.LoadPeopleResult peopleData) {
        Log.i(TAG,"sharique onResult called");
        if (peopleData.getStatus().getStatusCode() == CommonStatusCodes.SUCCESS) {
            PersonBuffer personBuffer = peopleData.getPersonBuffer();
            try {
                int count = personBuffer.getCount();
                for (int i = 0; i < count; i++) {
                    Log.d(TAG, "sharique Display name: " + personBuffer.get(i).getDisplayName());
                }
            } finally {
                personBuffer.release();
            }
        } else {
            Log.e(TAG, "sharique Error requesting visible circles: " + peopleData.getStatus());
        }
    }
    ...
    private void handleSignInResult(GoogleSignInResult result) {
        ...
        if (result.isSuccess()) {
            ...
            Log.i(TAG, "sharique google signin emailid is : " + emailid);
            User user = new User(Integer.parseInt(ufid), username, major, emailid);
